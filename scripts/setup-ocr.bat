...
rem --- Проверка ключа сразу ------------------------------------------
rem Неверный ключ иначе уедет в секреты молча, и выяснится это только на
rem первом распознанном файле — через выкладку, расписание и пять минут.
rem Мастер-ключ здесь тоже отсеивается: моделям он недоступен.
echo.
echo Проверяю ключ у маршрутизатора...

for /f "delims=" %%i in ('curl -s -o nul -w "%%{http_code}" ^
  "https://routerai.ru/api/v1/key" ^
  -H "Authorization: Bearer !LLM_KEY!"') do set "KEY_CODE=%%i"

if "%KEY_CODE%"=="401" (
  echo [!] Ключ не принят ^(401^). Это либо неверный ключ, либо мастер-ключ:
  echo     мастер-ключом управляют ключами, но обращаться к моделям нельзя.
  echo     Заведите обычный ключ API и запустите скрипт снова.
  goto :fail
)
...
if "%KEY_CODE%"=="200" (
  ...
  echo Маршрутизатор ответил %KEY_CODE% — продолжаю, но проверьте ключ,
  echo если распознавание не пойдёт.
)
