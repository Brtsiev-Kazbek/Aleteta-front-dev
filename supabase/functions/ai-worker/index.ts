 ...
 * можно допустить: ожидание ответа модели процессор не тратит вовсе. Страница
 * рисуется за десятую долю секунды работы, а потом полминуты идёт простой.
 *
 * Отсюда устройство бюджета. Страниц за запуск ограничиваем по процессору —
 * рисование единственное, что его ест. Момент остановки ограничиваем по часам
 * — и он общий, потому что исполнителя гасят по часам, а не по загрузке.
 *
 * Незаконченное задание не пропадает: оно возвращается в очередь, и следующий
 * запуск продолжает с первой нераспознанной страницы.
 */
const PAGE_BUDGET = Number(Deno.env.get("OCR_PAGES_PER_RUN") ?? 12);

/**
 * Момент, после которого исполнителя гасят.
 *
 * Сто тридцать секунд из полутора минут отведённых: остаток нужен, чтобы
 * успеть записать сделанное и вернуть задание в очередь. Обработчик по этому
 * числу решает и когда не начинать новую волну, и сколько ждать ответа —
 * поэтому оно одно, а не два несогласованных.
 */
const HARD_STOP_MS = Number(Deno.env.get("WORKER_RUN_MS") ?? 130_000);
...
export interface Run {
  /** Сколько страниц ещё можно нарисовать: это упирается в процессор. */
  pages: number;
  /** Момент, после которого исполнителя погасят. Это упирается в часы. */
  hardStop: number;
}
...
Deno.serve(async () => {
  ...
  const run: Run = { pages: PAGE_BUDGET, hardStop: Date.now() + HARD_STOP_MS };
  const processed: string[] = [];
  let unfinished = 0;

  for (let i = 0; i < BATCH; i += 1) {
    // Бюджет кончился — следующее задание достанется следующему запуску.
    if (run.pages <= 0 || Date.now() >= run.hardStop) break;
