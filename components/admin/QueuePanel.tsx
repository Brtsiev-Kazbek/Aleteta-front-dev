 ...
 *
 * ПОЧЕМУ КЛИЕНТСКИЙ. Данные приходят готовыми пропсами, и по ним панель могла бы
 * рисоваться на сервере. Мешает одна строка: `action={() => requeueFailedAction()}`.
 * Замыкание — обычная функция, а через границу «сервер → клиент» проходят только
 * серверные действия и простые значения, и Next отвечает на такое пятисотой:
 * «Functions cannot be passed directly to Client Components». Именно так эта
 * панель и легла при первом открытии раздела.
 *
 * Обойтись можно было бы, передав ссылку на само действие без обёртки, но тогда
 * ни один аргумент ему не передать — а фильтр по задаче здесь напрашивается
 * следующим шагом. Дешевле объявить панель клиентской.
 */
