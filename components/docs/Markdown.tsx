...
export function Markdown({ content }: { content: string }) {
  return (
    <ReactMarkdown
      ...
      components={{
        h1: ({ children }) => (
          <h1 className="mt-0 text-[2rem] font-medium leading-[1.15] tracking-[-0.03em] text-stone-900">
            {children}
          </h1>
        ),
        h2: ({ children }) => (
          <h2 className="mt-14 border-b border-stone-200 pb-2.5 text-[1.35rem] font-medium tracking-[-0.02em] text-stone-900">
            {children}
          </h2>
        ),
        h3: ({ children }) => (
          <h3 className="mt-10 text-[15px] font-medium tracking-[-0.01em] text-stone-900">
            {children}
          </h3>
        ),
        p: ({ children }) => (
        ...
        ul: ({ children }) => (
          <ul className="mt-5 flex flex-col gap-2.5 text-[15px] leading-relaxed text-stone-600">
            {children}
          </ul>
        ),
        ol: ({ children }) => (
          <ol className="mt-5 flex list-decimal flex-col gap-2.5 pl-5 text-[15px] leading-relaxed text-stone-600 marker:font-mono marker:text-[12px] marker:text-stone-400">
            {children}
          </ol>
        ),
        li: ({ children }) => (
        ...
        code: ({ children, className }) => {
          ...
          return (
            <code className="rounded border border-stone-200 bg-white px-1.5 py-0.5 font-mono text-[12.5px] text-stone-800">
              {children}
            </code>
          );
        },
        ...
        th: ({ children }) => (
          <th className="py-2.5 pr-6 font-mono text-[10px] font-normal uppercase tracking-[0.12em] text-stone-400">
            {children}
          </th>
        ),
        td: ({ children }) => (
          <td className="border-b border-stone-100 py-3 pr-6 align-top text-stone-600">
            {children}
          </td>
        ),
        blockquote: ({ children }) => (
          <blockquote className="mt-5 border-l-2 border-violet-300 pl-4 text-[15px] leading-relaxed text-stone-500">
            {children}
          </blockquote>
        ),
