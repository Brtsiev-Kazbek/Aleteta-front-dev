...
const ITEMS: QA[] = [
  ...
  },
  {
    ...
    answer:
      "Чат не помнит ваши дела и не проверяет реквизиты. Алетейя хранит контекст: какие файлы загружены, что в них написано, какие объекты заведены и где пробел. И главное — генерация пакета идёт по вашим шаблонам и опирается на данные, которые вы подтвердили, а не «сочиняется» заново каждый раз. Поэтому в реквизитах не бывает выдуманных значений.",
  },
  {
    id: "q3",
    question: "Насколько точна проверка? Можно ли доверять результату?",
    answer:
      "Проверка обязательных полей и форматов (кадастровый номер, ИНН, КПП, СНИЛС, паспорт) выполняется по строгим правилам, а не моделью — здесь ошибок нет по определению. Распознавание текста делает модель, и в сложных сканах она ошибается — поэтому оригинал всегда лежит рядом с расшифровкой. Итоговый документ всегда остаётся на вашей проверке: Алетейя убирает рутину, а не ответственность.",
  },
  {
    id: "q4",
