...
const Textarea = React.forwardRef<HTMLTextAreaElement, TextareaProps>(
  ({ className, ...props }, ref) => (
    <textarea
      ref={ref}
      className={cn(
        "flex min-h-[80px] w-full rounded-xl border border-line bg-surface px-3 py-2 text-body text-fg outline-none transition-colors placeholder:text-fg-faint focus:border-fg disabled:cursor-not-allowed disabled:opacity-50",
        className
      )}
      {...props}
    />
